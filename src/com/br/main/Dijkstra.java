...
public class Dijkstra {
	...
	public List<Vertice> encontrarMenorCaminhoDijkstraCusto(Grafo grafo, Vertice v1, Vertice v2) {
			...
			for (int i = 0; i < atual.getArestas().size(); i++) {
				...
				if (!vizinho.verificarVisita()) {
					...
					if (vizinho.getDistancia() > (atual.getDistancia() + atual
							.getArestas().get(i).getCusto())) {

						vizinho.setDistancia(atual.getDistancia()
								+ atual.getArestas().get(i).getCusto());
	...
	// Algoritmo de Dijkstra
	public List<Vertice> encontrarMenorCaminhoDijkstraDelay(Grafo grafo, Vertice v1, Vertice v2) {
		...
		while (!this.naoVisitados.isEmpty()) {
			...
			for (int i = 0; i < atual.getArestas().size(); i++) {
				...
				if (!vizinho.verificarVisita()) {
					...
					// distância
					if (vizinho.getDistancia() > (atual.getDistancia() + atual
							.getArestas().get(i).getDelay())) {

						vizinho.setDistancia(atual.getDistancia()
								+ atual.getArestas().get(i).getDelay());
						vizinho.setPai(atual);
